...
library(officer)
library(qs2) # used for faster Seurat data save and read access
library(stringr)
library(tools)
# load idw functions ----
...
if (if.error(nchar(data.file.name.old),
  ...
  message("Reading data ", data.file.name)
...
if (gene.type == "ensembl") {
  ...
  if (get.ensembl) {
    need.ensembl <- T
    while (need.ensembl){
      message("Trying to download ensembl data")
      need.ensembl <- is.error(
        ensembl <- useEnsembl(biomart = "genes", dataset = ensembl.dataset)  
        )
    }
    message("Ensembl data successfully downloaded")
  }
}
...
documentation.Word <- body_add_par(documentation.Word,
                                   ...
                                   style = "heading 2")
documentation.Word <- body_add_par(documentation.Word, 
                                   paste("data file: ", data.info[kidney.choice, "filename"]))
# get genes to plot ----
  ...
  if (gene.next == "exit") {
    ...
    if (data.info[kidney.choice, "gene.type"] == "gene") {
      if (is.element(gene.next, all.genes)) {
        ...
        documentation <- paste(gene.next, "added to list of genes to analyze")
...
cells.to.plot <- pick.cells.to.plot(data.set,0)
# create violin plot ----
...
# create dot plot ----
log1p <- get.input("Log1P transform dot plots?", c("Linear", "Log1P")) # let user determine whether to (ln(x+1) transform expression data
dot.plot <-
  DotPlot_scCustom(
    data.set,
    features = genes.to.plot,
...
if (log1p == "Linear") {
  dot.plot$data$avg.exp.scaled <- dot.plot$data$avg.exp
} # undoes log1p transformation of expression
dot.plot <- dot.plot +
  RotatedAxis() +
  scale_fill_discrete(breaks = genes.to.plot, labels = gene.names) +
  theme(
...
for (individual.gene in levels(genes)) {
  ...
  documentation.text <- paste0(
    "Which cells have ",
    sheet.gene.name,
    " expression in more than ",
    round(100 * expression.cutoff, 0),
    "% of cells"
  )
  ...
  for (cell.pointer in gene.data$cell) {
    if (gene.data[cell.pointer, "pct.exp"] > expression.cutoff) {
      ...
      pct.expression.text <- paste0(cell.pointer, ", ", round(100 * gene.data[cell.pointer, "pct.exp"], 0), "%")
...
if (length(all.cells.expressing.any.gene) > 0) {
  ...
  violin.2.plot <- format.ggplot(
    ...
    paste0(
      experiment.title,
      ": cell types with gene in more than ",
      round(100 * expression.cutoff, 0),
      "% of cells"
    )
  )
...
if (length(cells.to.plot.min.exp) > 0) {
  ...
  if (log1p == "Linear") {
    dot.plot.min.exp$data$avg.exp.scaled <- dot.plot.min.exp$data$avg.exp
  } # undoes log1p transformation of expression
